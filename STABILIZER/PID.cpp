...
float HELICOPTER_PID::get_ff(float requested_rate) const
{
...
float HELICOPTER_PID::get_leaky_i(float error, float dt, float leak_rate)
{
	if((_ki != 0) && (dt != 0)){
...
float S_P::get_p(float error) const
{
    return (float)error * _kp;
}
